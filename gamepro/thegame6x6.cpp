...
void setConfig2(int Board2[][6],bool Stat,bool Movement,int Score,int Highscore,bool ff)
{
    for(int i=0;i<6;i++)
        for(int j=0;j<6;j++)
...
void thegame6x6::update(QKeyEvent* event1)
{
        ...
        {
            setConfig2(Board2,stat,movement,score,0,ff);
...
void thegame6x6::setpics(){
   ...
   int k=Board2[0][0];
    ...
    if(k==4)
      ui->label_1->setStyleSheet(" background: rgb(2s37,224,200); color: rgb(119,110,101); font: bold; border-radius: 10px; font: 40pt; ");
    if(k==8)
